...
{$sans_tva = false}
{if $affaire["id_type_affaire"]}
	{if ATF::type_affaire()->select($affaire["id_type_affaire"], "assurance_sans_tva") == "oui"}
		{$sans_tva = true}
	{/if}
{/if}
		...
		'select':function(f,n,o){
			if(n.data.myId=='refi'){
				...
				{if $sans_tva !== false}
					...
					Ext.getCmp('facture[prix_sans_tva]').setValue({ATF::facture()->default_value("prix_sans_tva")});
				{else}
					Ext.getCmp('panel_hors_tva').hide();
					Ext.getCmp('facture[prix_sans_tva]').setValue(0);
				{/if}
