		...
		,listeners :{
			'select':function(f,n,o){
				if(o==1){
					...
					/*Une vente ne peux avoir un loyer unique*/
					if(Ext.getCmp('combodevis[type_contrat]').val()=='vente'){
						Ext.getCmp('combodevis[loyer_unique]').setValue('non');
						alert("Un contrat de vente ne peux avoir un loyer unique");
					}else{
						//Ext.getCmp('panel_vente').hide();
						Ext.getCmp('panel_loyer_lignes').hide();
						Ext.getCmp('panel_AR').hide();
						Ext.getCmp('panel_loyer_uniques').show();
						Ext.getCmp('devis[loyers]').enable();
						Ext.getCmp('devis[frais_de_gestion_unique]').enable();
						Ext.getCmp('devis[assurance_unique]').enable();
					}
				}
			}
		}
