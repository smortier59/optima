...
{if ATF::_r(id_devis)}
	{$calcul_prix='true'}
	{$q->reset()->addCondition("id_devis",classes::decryptId(ATF::_r(id_devis)))->addCondition("id_affaire_provenance",null,null,false,"IS NULL")->addCondition("visible","non","AND")->setView([order=>$fields])->end()}
...
{else}
	{$q->reset()->addCondition("id_commande",0)->addCondition("visible","non","AND")->addCondition("id_affaire_provenance",null,null,false,"IS NULL")->setView([order=>$fields])->end()}
	{include file="produits-update.tpl.js"}
{/if}	