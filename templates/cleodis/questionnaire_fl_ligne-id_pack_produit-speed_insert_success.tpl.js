,success:function(form, action) {
	if(action.result.result){
		if(Ext.ComponentMgr.get('{$id}').store && Ext.ComponentMgr.get('{$id}').store.getRange()){
			...
			records[idx].set('questionnaire_fl__dot__id_pack_produit_fk',action.result.result.data.id_pack_produit);
			records[idx].set('questionnaire_fl__dot__nom',action.result.result.nom);			
		}
