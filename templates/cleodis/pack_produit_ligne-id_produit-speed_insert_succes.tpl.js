,success:function(form, action) {
	if(action.result.result){
		if(Ext.ComponentMgr.get('{$id}').store && Ext.ComponentMgr.get('{$id}').store.getRange()){
			...
			records[idx].set('pack_produit__dot__id_fournisseur_fk',action.result.result.data.id_fournisseur);
			records[idx].set('pack_produit__dot__id_fournisseur',action.result.result.data.fournisseur);
		}
