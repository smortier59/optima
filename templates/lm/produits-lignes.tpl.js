							...
							{strip}
								<a href="javascript:;"
									onclick="{strip}
										...
										id: 'speed_insert{$id}',
										monitorResize:true,
										autoLoad:{ url: '{$key_class->name()},speed_insert_template.ajax,id={$id}&id_produit='+id_produit+'&parent_class={$current_class->table}', scripts:true }}).show();return false;{/strip}" >	<img class="png" height="16" width="16" alt="" src="{ATF::$staticserver}images/icones/insert.png"/>	</a>{/strip}{/capture}return '{$quickInsert|escape:javascript}';
					}
				},
			{/if}
			{
				...
				editor: jQuery.extend(
					{include file="generic-gridpanel-combo.tpl.js" key=id_produit function=null forceId="id_produit{$id}" extJSGridComboboxSeparator=true},
					{ listWidth:400 }
				)
			}, {
				hidden:true,
				...
				{if $current_class->table==facture_fournisseur_ligne ||  $current_class->table==parc}
						...
						header: 'Prix TTC',
						width:20,
						dataIndex: '{$current_class->table}__dot__prix_ttc',
						editor: new Ext.form.TextField({
							value:0
						})
					}, {
