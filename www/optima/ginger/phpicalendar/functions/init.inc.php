...
if (isset($_GET['cal']) && $_GET['cal'] !='') {
	...
	if (!is_array($_GET['cal'])){
		$liste = explode(',', $_GET['cal']);
		//on va chercher l'id_user correspondants au nom
		$c_user=new classes('user');
		unset($_GET['cal']);
		foreach($liste as $key=>$item){
			$c_user->q->reset()->addCondition("REPLACE( CONCAT( nom, '_', prenom ) , ' ', '' )",$item)->addField('id_user')->setStrict();
			$id_user=$c_user->select_all();
			$_GET['cal'][$id_user[0]["id_user"]]=$item;
		}
	}
...
if (count($local_cals) > 0) {
	...
	$cal_filelist = $cal_filelist+$local_cals;
