...
function list_user($liste_user=NULL){
	$c_user=new classes('user');
	$c_user->q->setStrict()->addField('id_user')->addField('nom')->addField('prenom');

	foreach($c_user->select_all() as $key=>$item){
		$return .="<input type='checkbox' name='cal[".$item["id_user"]."]' value='".str_replace(" ","",$item['nom']."_".$item['prenom'])."' ".($liste_user?($liste_user[$item["id_user"]]?"checked=checked":""):"checked=checked").">".str_replace(" ","",$item['nom']."_".$item['prenom'])."<br />";
	}
