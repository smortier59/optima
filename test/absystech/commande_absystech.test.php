...
class commande_absystech_test extends ATF_PHPUnit_Framework_TestCase {
	...
	function testAnnulee(){
		...
		$this->assertEquals(array(0=>array("msg"=>"notice_commande_annulee","title"=>"Succès !","timer"=>"","type"=>"success")),ATF::$msg->getNotices(),"3 La notice d'annulation fonctionne bien");
	...
	function test_select_all(){
		$this->obj->q->reset()->setCount()->setLimit(5);
		...
		$data = array("id_commande"=>$r[0]["commande.id_commande"] , "etat"=>"annulee");

		$this->obj->u($data);

		ATF::$usr->set('id_profil',2);

		$this->obj->q->reset()->setCount()->where('commande.id_commande',$data['id_commande']);
		$r = $this->obj->select_all();
		$r = $r['data'];
	...
	function test_setInfos(){
		...
		$params = array('id_commande'=>$r[0]['commande.id_commande'],'etat'=>'facturee');
		$this->obj->setInfos($params);


		$r = $this->obj->select($r[0]['commande.id_commande']);
	...
	function testInsert(){
		...
		unset($commande[$this->obj->table]["id_affaire"]);
		$commande[$this->obj->table]["date"]=date("Y-m-d");
		$commande[$this->obj->table]["resume"]="TU commande insert";

		$id_commande = $this->obj->insert($commande,$this->s);

		$this->assertNotNull($id_commande,"Insertion de la commande incorrecte 1");

		$this->obj->d($id_commande);
		...
		echo "\n 2 ".ATF::db()->numberTransaction();

		$this->assertEquals(600,$erreur_trouvee3,"ERREUR 3 NON ATTRAPPEE (resume non insere)");
		*/
