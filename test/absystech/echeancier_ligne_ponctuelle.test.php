<?
class echeancier_ligne_ponctuelle_test extends ATF_PHPUnit_Framework_TestCase {
	// @author Cyril Charlier <ccharlier@absystech.fr>
	private $echeancier_ligne_ponctuelle;
	private $echeancier;
	private $societe;
	...
	private function environnement_test(){
		ATF::db()->truncate("echeancier_ligne_ponctuelle");
		// societe 
		...
		$this->echeancier_ligne_ponctuelle =ATF::echeancier_ligne_ponctuelle()->insert(array(
			"designation" =>"echeancier ligne ponctuelle test TU", 
			'quantite'=>2,
			'puht'=> 5.2,
	...
	public function test_table(){
		$this->assertEquals("echeancier_ligne_ponctuelle",$this->obj->name(),"Probleme de nom de classe echeancier_ligne_ponctuelle");
	}

	public function test_GET(){
		$ret =ATF::echeancier_ligne_ponctuelle()->_GET();
		$this->assertEquals("echeancier ligne ponctuelle test TU",$ret[0]["designation"],"Probleme de message retour designation echeancier ligne periodique");
	...
	public function test_GETWithSearch(){
		...
		ATF::echeancier_ligne_ponctuelle()->insert(array(
		...
		$ret =ATF::echeancier_ligne_ponctuelle()->_GET($get);		
	...
	public function test_GETById(){
		...
		$maLignePeriodique = ATF::echeancier_ligne_ponctuelle()->insert(array(
	...
	public function test_PUT(){
		$post = array(
			...
			'id_echeancier_ligne_ponctuelle' => $this->echeancier_ligne_ponctuelle
		);
		$ret =ATF::echeancier_ligne_ponctuelle()->_PUT(false,$post);
	...
	public function test_PUTError(){
		try {
			...
			ATF::echeancier_ligne_ponctuelle()->_PUT('',$post);
		} catch (errorATF $e) {
			$error = $e->getCode();
		}
	...
	public function test_DELETEWithoutId(){
		try {
			$get = array('test'=> 'test');
			ATF::echeancier_ligne_ponctuelle()->_DELETE($get);
		} catch (errorATF $e) {
			$error = $e->getCode();
		}
		$this->assertEquals(1000,$error,'problème sur le delete lorsque l\'id est manquant');
	}

	public function test_DELETE(){
		$get = array('id'=> $this->echeancier_ligne_ponctuelle);
		$ret = ATF::echeancier_ligne_ponctuelle()->_DELETE($get);
