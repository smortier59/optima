...
//		ATF::profil()->u(array("id_profil"=>ATF::$usr->get("id_profil"),"seuil"=>""));
		...
		$this->assertEquals(array(
									0=>array(
										"msg"=>"En tant que Associé vous n'avez pas les droits de faire un devis supérieur à 1 €. Si vous pensez qu'il s'agit d'une erreur, contactez votre supérieur hiérarchique pour qu'il vérifie les droits de votre profil utilisateur.","title"=>"Droits d'accès requis pour cette opération ! ","timer"=>""
										)
	...
	function testPerdu(){
		...
		$this->assertEquals(array(),ATF::$msg->getNotices(),"5 La notice devrait être vide");
	...
	function testSelect_all(){
		...
		unset($r[0]['allowCmd'],$r[0]['allowUnlockDevis'],$r[0]['allowCancel'],$r[0]['devis.id_devis'],$r[0]['devis.etat']);
		ATF::devis()->u($r[0]);
	...
	function testAnnule(){
		...
		$this->assertEquals(array(0=>array("msg"=>"Le devis '".$this->obj->select($last_id,"ref")."' a bien été annulé","title"=>"Succès !","timer"=>"")),ATF::$msg->getNotices(),"3 La notice en état annulé ne se fait pas");
		...
		$this->assertEquals(array(0=>array("msg"=>"Le devis '".$this->obj->select($devis["etat"],"ref")."' a bien été annulé","title"=>"Succès !","timer"=>"")),ATF::$msg->getNotices(),"7 La notice en état annulé ne se fait pas");
