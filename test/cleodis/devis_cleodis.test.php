...
//		echo ATF::db()->numberTransaction().")";
    ...
    */
	...
	public function test_defaultValuesExactitude(){
		$c=new devis_exactitude();
		
		$this->assertEquals("recrutement" ,$c->default_value("type_contrat") , "Default value Exactitude 1 incorrect");
		$this->assertEquals("" ,$c->default_value("ref") , "Default value Exactitude 2 incorrect");
	}
	...
	public function test_insertExactitude(){
		...
		$infos["devis"]["intitule_poste"] = "intitule du poste";
		$id_devis = ATF::devis_exactitude()->insert($infos); 

		$this->beginTransaction("cleodis", false, true);

		$this->assertEquals("Merci de renseigner tout les champs obligatoires ( intitule_poste)", $msg, "Erreur non apparue ??");

		$this->assertNotNull($id_devis , "Devis non insere??");

	}



	public function test_insertAuditExactitude(){
		$this->beginTransaction("exactitude", true, false);
		...
		$this->assertEquals("Il faut au moins une ligne de prestation !", $msg, "Erreur non apparue ??");

		$this->assertNotNull($id_devis , "Devis non insere??");

	}

	public function test_updateExactitude(){
		$this->beginTransaction("exactitude", true, false);
		...
		$devis["id_devis"] = $id_devis;
		$devis["duree_exclu"] = "3 mois";

		$id_devis = ATF::devis_exactitude()->update(array("preview"=>true,"devis"=>$devis));

		$devis["id_devis"] = $id_devis;

		$id_devis = ATF::devis_exactitude()->update(array("devis"=>$devis));
