...
class facture_cleodis_test extends ATF_PHPUnit_Framework_TestCase {
   ...
   public function test_contientFactureRejetee(){
    ...
    public function test_default_value_exactitude(){
        ...
        $this->assertEquals(0,$c->default_value("prix"),'valeur prix 1 avant remplissage du request');
        $this->assertEquals(ATF::$usr->get('id_user'),$c->default_value("id_user"),'valeur id_user avant remplissage du request');

        ATF::_r('id_devis',13250);

        $this->assertEquals(16,$c->default_value("id_user"),'valeur id_user avant remplissage du request');
        $this->assertEquals(6682,$c->default_value("id_societe"),'valeur id_societe avant remplissage du request');
        $this->assertEquals(1.2,$c->default_value("tva"),'valeur tva avant remplissage du request');
        $this->assertEquals("virement",$c->default_value("mode_paiement"),'valeur mode_paiement avant remplissage du request');
        
        ATF::_r('id_facture',95059);
        $this->assertEquals(date("Y-m-d"),$c->default_value("date"),'valeur date avant remplissage du request');
        $this->assertEquals(13288,$c->default_value("id_affaire"),'valeur prix 2 avant remplissage du request');
        $this->assertEquals(1800.00,$c->default_value("prix"),'valeur prix 2 avant remplissage du request');

        $this->assertEquals(NULL,$c->default_value("ref"),'valeur ref avant remplissage du request');
    ...
    public function test_select_all_exactitude(){
        $this->beginTransaction("exactitude");
        $c = new facture_exactitude();  
        
        $c->q->reset()->where("id_facture", 95059);
        ...
        $data =  array(array(
            "societe.code_client" => NULL,
            "societe.code_client_fk" => 6682,
            "facture.id_facture" => 95059
        ));

        $this->assertEquals($data,$res,'select_all_exactitude incorrect');

    }


    public function test_export_special2_exactitude(){

        $this->beginTransaction("exactitude");
        $c = new facture_exactitude();  

        $c->q->reset()->where("facture.date","2015-12-17","AND",NULL,"<=");

        ob_start();  
        $c->export_special2(array());
        ob_end_clean();     
    ...
    public function test_export_special_exactitude(){
    ...
    public function test_insert_exactitude_1(){
        $this->beginTransaction("exactitude");
        $c = new facture_exactitude();  
    ...
    public function test_insert_exactitude_2(){
        $this->beginTransaction("exactitude");
        $c = new facture_exactitude();  
