...
class produit_cleodis extends produit {
	...
	*/
    ...
    public function export_middleware($infos,$s){
        ...
        // Filtre par date pour éviter la volumétrie
        ATF::pack_produit()->q->setLimit(750);
        ATF::pack_produit()->q->addOrder('id_pack_produit', 'DESC');


        $packs = ATF::pack_produit()->select_all();
        ...
        foreach ($packs as $k=>$pack) {
        	ATF::pack_produit_ligne()->q->where('id_pack_produit',$pack['id_pack_produit']);
        }
        $lignes = ATF::pack_produit_ligne()->select_all();
        ...
        foreach ($lignes as $k=>$ligne) {
        	ATF::produit()->q->where('id_produit',$ligne['id_produit']);
        }
        $produits = ATF::produit()->select_all();
