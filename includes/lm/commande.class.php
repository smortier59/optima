...
class commande_lm extends commande {
	/**
	* Constructeur
	*/
	function __construct($table_or_id=NULL) {
		...
		$this->colonnes['fields_column'] = array(
			'commande.ref'
			,'specificDate'=>array("custom"=>true,"nosort"=>true,"renderer"=>"dateCleCommande","width"=>290)
//			,'specificDateRestitution'=>array("custom"=>true,"nosort"=>true,"renderer"=>"dateCleCommandeRestitution","width"=>290)
			...
			,'commande.etat'
			,'files'=>array("custom"=>true,"nosort"=>true,"renderer"=>"pdfCommande","width"=>100)
			,'courriers'=>array("custom"=>true,"nosort"=>true,"renderer"=>"pdfCourriers","width"=>100)
			,'retour'=>array("custom"=>true,"nosort"=>true,"type"=>"file","renderer"=>"uploadFile","width"=>60)
			,'retourPV'=>array("custom"=>true,"nosort"=>true,"type"=>"file","renderer"=>"uploadFile","width"=>60)
			,'demandeResi'=>array("custom"=>true,"nosort"=>true,"type"=>"file","align"=>"center","renderer"=>"uploadFile","width"=>70)
			,'actions'=>array("custom"=>true,"nosort"=>true,"renderer"=>"cmdAction","width"=>200)
		);
		...
		switch ($infos["field"]) {
			case "date_debut":
				...
				if ($infos["date"]) {
					...
					ATF::facturation_fournisseur()->createFactureFournisseurAchat($affaire);
...
// @todo A vérifier, normalement les 3 dates contrat, prel, et PV ne doivent pas modifier l'état du contrat !
	...
	/**
	* Met à jour l'état de la commande ainsi que l'état de la commnade des affaires parentes (dans la modif checkAndUpdateDate) ou des affaires filles (dans lmStatut)
	* @author Mathieu TRIBOUILLARD <mtribouillard@absystech.fr>
	*/
		...
		if ($return = parent::set($attribute,$value)) {
			switch ($attribute) {
				case "etat":
					...
					if ($value=="arreter") {
						if(ATF::$codename == "lmbe") $a = new affaire_lmbe($this->get('id_affaire'));
						else $a = new affaire_lm($this->get('id_affaire'));
				...
				if($widget){
					...
					if($type == "o2m"){
						$this->q->addCondition("societe.code_client",'%M%',"OR","nonFinie","LIKE")
								...
								->addCondition("societe.code_client",'%B%',"OR","nonFinie","LIKE")
						...
						$this->q->addCondition("societe.code_client",'%M%',"AND","nonFinie","NOT LIKE")
								...
								->addCondition("societe.code_client",'%B%',"AND","nonFinie","NOT LIKE")
