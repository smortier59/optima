...
class affaire_exquado extends affaire {
...
//	}
    ...
    */   	
	public function updateDate($infos,&$s,&$request){
		...
		if(isset($infos['value']) && $infos['id_affaire']){
			...
			$affaire = new affaire_exquado($infos['id_affaire']);
			try {
				switch ($infos["field"]) {
	...
	*/
	function getParentAvenant($id_affaire=NULL){
		...
		if($id_parent)	{
			return new affaire_exquado($id_parent);
		}else{
			return false;
		}
	}
	...
	function getCommande($id_affaire=NULL){
		...
		if($id_affaire){
			...
			if($id_commande = ATF::commande()->sa()) {
				return new commande_exquado($id_commande);
			}else{
				return false;
			}
		}else{
	...
	function getDevis($id_affaire=NULL){
		...
		if($id_affaire){
			...
			if($id_devis = ATF::devis()->sa()) {
				return new devis_exquado($id_devis);
			}else{
				return false;
			}
		}else{
...
//log::logger("f->PV(".$taux."/".$freq[$loyer["frequence_loyer"]]."/100, ".$loyer["duree"].", ".$loyer["loyer"].", ".$vr." , 1);",ygautheron);
    ...
    */
	public function getCompteTLoyerActualise(&$infos) {		
		$a = new affaire_exquado($infos["id_affaire"]);
...
class affaire_midas extends affaire_exquado {
	function __construct($table_or_id=NULL) {
		$this->table = "affaire"; 
		parent::__construct($table_or_id);
