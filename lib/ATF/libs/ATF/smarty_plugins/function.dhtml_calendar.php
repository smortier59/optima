...
*/
function smarty_function_dhtml_calendar($params, $smarty, $template)
{
    ...
    static $all = false;
    if (!$all) $all = array_merge($requires, $allowed, $deprecated);
    ...
    foreach ($_params as $field=>$value) {
        ...
        else if (!is_numeric($value) && !($all[$field] == 'function'))
            $value = '"'.$value.'"';
        if ($_params_out)
            $_params_out .= ',';
