...
class awGDDriver extends Driver {
	...
	private function drawImage(awImage $image, $return = FALSE, $header = TRUE) {
		...
		/* ATF MODIFICATION spéciale pour les TU sinon il y a des infos qui s'affiche et qui vrac le lancement des TU */
		/* Original : remettre le ob_start() dans le if */
		ob_start();
		if($return) {
		}
		...
		/* ATF MODIFICATION spéciale pour les TU sinon il y a des infos qui s'affiche et qui vrac le lancement des TU*/
		/* Original : remettre le ob_get_clean() dans le if */
		return ob_get_clean();
		if($return) {
		}
	}
