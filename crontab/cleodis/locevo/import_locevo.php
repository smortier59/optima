	...
	try {
		...
		while ($ligne = fgetcsv($fpr, 0, ';')) {
			...
			if ($alreadyExistsFromRef /*|| $alreadyExistsFromEan*/) {
				log::logger('Skipping EAN/REF found : ' . print_r($alreadyExistsFromRef,true) /*." || ". print_r($alreadyExistsFromEan,true)*/, "import_locevo_escape_product");
				log::logger("Produit ".$ref."/".$ean." non traité car déjà présent dans la BDD.", "import_locevo_escape_product");
				continue;
			}
		...
		log::logger("#####Produits imports",  "locevo_migration");
