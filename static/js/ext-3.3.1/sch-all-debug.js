...
Ext.ux.form.DateTime = Ext.extend(Ext.form.Field, {
    ...
    initComponent: function() {
        ...
        this.df = new Ext.form.DateField(a);
        ...
        this.tf = new Ext.form.TimeField(b);
    ...
    onRender: function(a, b) {
        ...
        Ext.ux.form.DateTime.superclass.onRender.call(this, a, b);
        var t;
        if ('below' === this.timePosition || 'bellow' === this.timePosition) {
            t = Ext.DomHelper.append(a, {
                tag: 'table',
        ...
        if ('side' === this.msgTarget) {
            var c = this.el.findParent('.x-form-element', 10, true);
            this.errorIcon = c.createChild({
    ...
    setValue: function(a) {
        ...
        if ('number' === typeof a) {
            a = new Date(a)
            } else if ('string' === typeof a && this.hiddenFormat) {
            a = Date.parseDate(a, this.hiddenFormat)
            }
    ...
    renderer: function(c) {
        var d = c.editor.dateFormat || Ext.ux.form.DateTime.prototype.dateFormat;
        d += ' ' + (c.editor.timeFormat || Ext.ux.form.DateTime.prototype.timeFormat);
        var e = function(a) {
            var b = Ext.util.Format.date(a, d);
            return b
        };
        return e
    }
...
Ext.override(Ext.form.TimeField, {
    reconfigure: function(a, b, c) {
        ...
        this.increment = c;
        var d = this.parseDate(this.minValue);
        if (!d) {
            d = new Date(this.initDate).clearTime()
            }
        var e = this.parseDate(this.maxValue);
        if (!e) {
            e = new Date(this.initDate).clearTime().add('mi', (24 * 60) - 1)
            }
        var f = [];
        while (d <= e) {
            f.push([d.dateFormat(this.format)]);
            d = d.add('mi', this.increment)
            }
...
Sch.util.Date = {
    betweenLesser: function(a, b, c) {
        var t = a.getTime();
        return b.getTime() <= t && t < c.getTime()
        },
    constrain: function(a, b, c) {
        return Sch.util.Date.min(Sch.util.Date.max(a, b), c)
        },
    add: function(a, b, c) {
        var d = a.clone();
        if (!b || c === 0)
        ...
        case this.MILLI:
            ...
            break;
        case this.SECOND:
            d = new Date(a.getTime() + (c * 1000));
            break;
        case this.MINUTE:
            d = new Date(a.getTime() + (c * 60000));
            break;
        case this.HOUR:
            d = new Date(a.getTime() + (c * 3600000));
            break;
        case this.DAY:
            d.setDate(a.getDate() + c);
            break;
        case this.WEEK:
            d.setDate(a.getDate() + c * 7);
            break;
        case this.MONTH:
            var e = a.getDate();
            if (e > 28) {
                e = Math.min(e, this.add(a.getFirstDateOfMonth(), this.MONTH, c).getLastDateOfMonth().getDate())
                }
            d.setDate(e);
            d.setMonth(d.getMonth() + c);
            break;
        case this.QUARTER:
            d = this.add(a, this.MONTH, 3);
            break;
        case this.YEAR:
            d.setFullYear(a.getFullYear() + c);
            break
        }
        return d
    },
    ...
    getDurationInUnit: function(a, b, c) {
        var d;
        switch (c) {
        case this.QUARTER:
            d = Math.round(Sch.util.Date.getDurationInMonths(a, b) / 3);
            break;
        case this.MONTH:
            d = Math.round(Sch.util.Date.getDurationInMonths(a, b));
            break;
        case this.WEEK:
            d = Math.round(Sch.util.Date.getDurationInDays(a, b)) / 7;
            break;
        case this.DAY:
            d = Math.round(Sch.util.Date.getDurationInDays(a, b));
            break;
        case this.HOUR:
            d = Math.round(Sch.util.Date.getDurationInHours(a, b));
            break;
        case this.MINUTE:
            d = Math.round(Sch.util.Date.getDurationInMinutes(a, b));
            break;
        case this.SECOND:
            d = Math.round(Sch.util.Date.getDurationInSeconds(a, b));
            break;
        case this.MILLI:
            d = Math.round(Sch.util.Date.getDurationInMilliseconds(a, b));
            break
        }
        return d
    },
    ...
    getDurationInBusinessDays: function(a, b) {
        ...
        d;
        for (var i = 0; i < c; i++) {
            d = Sch.util.Date.add(a, Sch.util.Date.DAY, i).getDay();
            if (d !== 6 && d !== 0) {
                nbrBusinessDays++
            }
        }
    ...
    intersectSpans: function(a, b, c, d) {
        return this.betweenLesser(a, c, d) || this.betweenLesser(c, a, b)
        },
    MILLI: Date.MILLI,
    SECOND: Date.SECOND,
    MINUTE: Date.MINUTE,
...
Ext.apply(Sch.util.Date, {
    compareUnits: (function() {
        ...
        ind2;
        return function(a, b) {
            ind1 = units.indexOf(a);
            ind2 = units.indexOf(b);
...
Sch.AbstractSchedulerPanel = Ext.extend(Ext.grid.EditorGridPanel, {
    ...
    setReadOnly: function(a) {
        this.readOnly = a;
        this.el[a ? 'addClass': 'removeClass'](this.cmpCls + '-readonly')
        },
    ...
    getXYFromDate: function(a) {
        return this.view.getXYFromDate(a)
        },
    getDateFromDomEvent: function(e, a) {
        return this.getDateFromXY(e.getXY(), a)
        },
    getDateFromX: function(x, a) {
        return this.getDateFromXY([x, 0], a)
        },
    getDateFromXY: function(a, b) {
        var c = this.view.getDateFromXY(a);
        if (c && b) {
            c = this[b + 'Date'](c)
            }
        return c
    ...
    roundDate: function(a) {
        return this.timeAxis.roundDate(a)
        },
    floorDate: function(a) {
        return this.timeAxis.floorDate(a)
        },
    ceilDate: function(a) {
        return this.timeAxis.ceilDate(Date)
        },
    getFormattedDate: function(a) {
        return a.format(this.getDisplayDateFormat())
        },
    getFormattedEndDate: function(a, b) {
        var c = this.timeAxis,
        resUnit = c.getResolution().unit;
        if (resUnit in this.largeUnits && a.getHours() === 0 && a.getMinutes() === 0 && !(a.getYear() === b.getYear() && a.getMonth() === b.getMonth() && a.getDate() === b.getDate())) {
            a = Sch.util.Date.add(a, Date.DAY, -1)
            }
        return a.format(this.getDisplayDateFormat())
        },
    ...
    setTimeResolution: function(a, b) {
        this.timeAxis.setResolution(a, b);
    ...
    afterRender: function() {
        ...
        var a = this.getView();
        this.relayEvents(a, ['timeheaderdblclick']);
        if (this.overClass) {
    ...
    setupScheduleEvents: function() {
        this.mon(this.getView().mainBody, {
            click: function(e) {
                ...
                if (t) {
                    var a = this.getDateFromDomEvent(e, 'floor');
                    this.fireEvent('scheduleclick', this, a, this.view.findRowIndex(t), e)
                    }
            },
            dblclick: function(e) {
                var t = e.getTarget('td.sch-timetd', 2);
                if (t) {
                    var a = this.getDateFromDomEvent(e, 'floor');
                    this.fireEvent('scheduledblclick', this, a, this.view.findRowIndex(t), e)
                    }
            },
            contextmenu: function(e) {
                ...
                if (t) {
                    var a = this.getDateFromDomEvent(e, 'floor');
                    this.fireEvent('schedulecontextmenu', this, a, this.view.findRowIndex(t), e)
                    }
            },
            scope: this
        }, this)
        },
    switchViewPreset: function(a, b, c, d) {
        if (this.fireEvent('beforeviewchange', this) !== false) {
            if (Ext.isString(a)) {
                a = Sch.PresetManager.getPreset(a)
                }
            if (!a) {
            ...
            if (d) {
                ...
                e.start = b || this.timeAxis.getStart();
                e.end = c
            }
            this.timeAxis.reconfigure(e)
            }
    },
    ...
    initComponent: function() {
        ...
        Ext.each(this.getColumnModel().config, function(a) {
            a.setEditor = Ext.emptyFn
        })
        },
    tipCfg: {
        cls: 'sch-tip',
            ...
            listeners: {
                beforeshow: {
                    fn: function(a) {
                        ...
                        var b = this.getTooltipRecord(a.triggerElement);
    ...
    getTooltipRecord: function(a) {
        return this.getEventRecordFromDomId(a.id)
        },
    constructor: function(a) {
        this.addEvents('beforetooltipshow', 'beforeviewchange', 'viewchange', 'timeheaderdblclick', 'scheduleclick', 'scheduledblclick', 'schedulecontextmenu');
        Sch.AbstractSchedulerPanel.superclass.constructor.call(this, a)
        },
...
Sch.PresetManager = Ext.apply(new Ext.util.MixedCollection(), {
    registerPreset: function(a, b) {
        if (b) {
            var c = b.headerConfig;
            for (var o in c) {
                if (c.hasOwnProperty(o)) {
                    if (Sch.util.Date[c[o].unit]) {
                        c[o].unit = Sch.util.Date[c[o].unit.toUpperCase()]
                        }
                }
            }
            if (b.timeResolution && Sch.util.Date[b.timeResolution.unit]) {
                b.timeResolution.unit = Sch.util.Date[b.timeResolution.unit.toUpperCase()]
                }
            if (b.shiftUnit && Sch.util.Date[b.shiftUnit]) {
                b.shiftUnit = Sch.util.Date[b.shiftUnit.toUpperCase()]
                }
        }
        if (this.isValidPreset(b)) {
            this.add(a, b)
            } else {
    ...
    isValidPreset: function(a) {
        ...
        for (var o in a.headerConfig) {
            if (a.headerConfig.hasOwnProperty(o)) {
                valid = valid && validUnits.indexOf(a.headerConfig[o].unit) >= 0
            }
        }
        if (a.timeResolution) {
            valid = valid && validUnits.indexOf(a.timeResolution.unit) >= 0
        }
        if (a.shiftUnit) {
            valid = valid && validUnits.indexOf(a.shiftUnit) >= 0
        }
...
Sch.AbstractSchedulerView = Ext.extend(Ext.grid.GridView, {
    ...
    translateToPageCoordinate: function(a) {
        return [a[0] + this.mainBody.getLeft(), a[1] + this.mainBody.getTop()]
        },
    ...
    getColumnHeaderCls: function(i, a) {
        ...
        return b + (this.cm.config[i].hCls || '') + (this.cm.config[i].headerCls || '')
        },
    ...
    getSingleUnitInPixels: function(a) {
        return Sch.util.Date.getUnitToBaseUnitRatio(this.getColumnModelUnit(), a) * this.cm.getColumnWidth(this.leftColumns.length)
        },
    ...
    resolveHeaderGroupCell: function(a) {
        ...
        if (b) {
            var c;
            if (b.match(/ux-grid-hd-group-row-top/)) {
                c = "top"
            } else {
                c = "middle"
            }
    ...
    calculateTimeColumnWidth: function(a) {
        ...
        var c = Sch.util.Date.getMeasuringUnit(timelineUnit);
        ratio = Math.min(ratio, Sch.util.Date.getUnitToBaseUnitRatio(timelineUnit, c));
    ...
    getXFromDate: function(a) {
        ...
        if (tick >= 0) {
            ...
            x = colWidth + (b < this.cm.getColumnCount() ? this.cm.getColumnWidth(b) * (tick - Math.floor(tick)) : 0)
            }
        return x
    },
    ...
    getHeaderRowCells: function(a) {
        ...
        for (i = 0, len = r.length; i < len; i++) {
            ...
            if (i === 0 && b) {
                ...
                width = this.getHeaderGroupCellWidth(group.start, group.end, headerConfig[a].unit, headerConfig[a].increment || 1);
                cls = 'sch-timeheader ';
                if (width <= 0) {
                    cls += ' x-hidden '
                }
    ...
    getHeaderGroupCellWidth: function(a, b, c, d) {
        var e = this.getColumnModelUnit(),
        baseIncrement = this.getColumnModelIncrement(),
        width,
        measuringUnit = Sch.util.Date.getMeasuringUnit(e),
        durationInMeasuringUnit = Sch.util.Date.getDurationInUnit(a, b, measuringUnit);
        if (this.timeAxis.isContinuous()) {
            width = durationInMeasuringUnit * this.getSingleUnitInPixels(measuringUnit) / baseIncrement
        } else {
            width = this.getXFromDate(b) - this.getXFromDate(a)
            }
            ...
            for (var r in headerRows) {
                if (headerRows.hasOwnProperty(r)) {
                    if (this.leftColumns.length > 0) {
                        headerRows[r].unshift({
                            colspan: this.leftColumns.length
                        })
                        }
                    if (this.rightColumns.length > 0) {
                        headerRows[r].push({
                            colspan: this.rightColumns.length
                        })
                        }
                }
            }
    ...
    isValidHeaderConfig: function() {
        ...
        valid = true;
        if (a.bottom) {
            cmpResult = Sch.util.Date.compareUnits(a.bottom.unit, a.middle.unit);
            if (cmpResult > 0 || (cmpResult === 0 && (a.bottom.increment || 1) > (a.middle.increment || 1))) {
                valid = false
            }
        }
        if (a.top) {
            cmpResult = Sch.util.Date.compareUnits(a.middle.unit, a.top.unit);
            if (cmpResult > 0 || (cmpResult === 0 && (a.middle.increment || 1) > (a.top.increment || 1))) {
                valid = false
            }
        }
        return valid
    ...
    init: function(a) {
        ...
        this.timeAxis.on('reconfigure', function() {
...
Sch.HorizontalSchedulerView = Ext.extend(Sch.AbstractSchedulerView, {
    ...
    barMargin: 1,
    constructor: function(a) {
        Sch.HorizontalSchedulerView.superclass.constructor.call(this, a);
        if (this.forceFit) {
    ...
    onEventAdd: function(s, a) {
        ...
        for (var i = 0, l = a.length; i < l; i++) {
            b = this.grid.getResourceByEventRecord(a[i]);
            if (b) {
                this.refreshRow(b)
                }
        }
    ...
    init: function(a) {
        Ext.apply(this, {
            timeCellRenderer: a.timeCellRenderer,
            eventBodyTemplate: a.eventBodyTemplate,
            eventTemplate: a.eventTemplate,
            eventRenderer: a.eventRenderer,
            ...
            if (c < b.length - 1) {
                ...
                for (var i = 0; i < d.length; i++) {
                    if (i === 0 && leftGroupWidth > 0) {
                        d[i].style.width = leftGroupWidth + 'px'
                    } else if (i === d.length - 1 && rightGroupWidth > 0) {
                        d[i].style.width = rightGroupWidth + 'px'
                    } else {
                        var e = headerConfig.top ? (c === 0 ? 'top': 'middle') : 'middle',
                        group = a[e][i],
                        width = this.getHeaderGroupCellWidth(group.start, group.end, headerConfig[e].unit, headerConfig[e].increment || 1);
    ...
    renderHeaders: function() {
        ...
        if (headerRows) {
            if (headerRows.top) {
                var b = this.getHeaderRowCells('top');
            ...
            if (headerRows.middle) {
                var c = this.getHeaderRowCells('middle');
                headers.push(a.header.apply({
                    tclass: 'middle',
                    tstyle: 'width:' + tw,
                    cells: c.join('')
        ...
        for (i = 0; i < len; i++) {
            ...
            cb[cb.length] = d.apply(p)
            }
        headers.push(a.header.apply({
            cells: cb.join(''),
            tstyle: 'width:' + tw + ';',
            tclass: headerRows.middle ? 'bottom': 'middle'
        }));
        return headers.join('')
        },
    ...
    timeColumnRenderer: function(v, m, a, b, c, d, e) {
        var f = '&#160;',
        scheduler = this.grid,
        cm = this.cm,
        ta = this.timeAxis,
        ...
        tickIndex = c - firstScheduleColumnIndex,
        prevEnd = (c === firstScheduleColumnIndex) ? null: ta.getAt(tickIndex - 1).end,
        colWidth = cm.getColumnWidth(c),
        colTick = ta.getAt(tickIndex),
        colStart = colTick.start,
        colEnd = colTick.end;
        this.timeCellRenderer.call(scheduler, e, m, a, b, c, d, colStart, colEnd);
        for (var i = 0, l = e.getCount(); i < l; i++) {
            var g = e.items[i],
            start = g.data.StartDate,
            end = g.data.EndDate;
            if (start && end) {
                var h;
                if (c === firstScheduleColumnIndex) {
                    h = Sch.util.Date.betweenLesser(start, colStart, colEnd) || (start < colStart && end > colStart)
                    } else {
                    h = Sch.util.Date.betweenLesser(start, prevEnd, colEnd) && ((end > colStart) || end - start === 0)
                    }
                if (h) {
                    var j = colEnd - colStart,
                    leftOffset = ((Sch.util.Date.max(start, colStart) - colStart) / j) * colWidth,
                    itemWidth = this.getXFromDate(Sch.util.Date.min(end, viewEnd)) - this.getXFromDate(Sch.util.Date.max(start, viewStart)),
                    tplData = {
                        id: g.id,
                        internalcls: (g.dirty ? 'sch-dirty': '') + (end > viewEnd ? ' sch-event-endsoutside ': '') + (start < viewStart ? ' sch-event-startsoutside': ''),
                        left: leftOffset,
                    ...
                    if (this.eventRenderer) {
                        var k = this.eventRenderer.call(scheduler, g, a, tplData, b, c);
                        if (Ext.isObject(k) && this.eventBodyTemplate) {
                            tplData.body = this.eventBodyTemplate.apply(k)
                            } else {
                            tplData.body = k
                        }
                    } else if (this.eventBarTextField) {
                        tplData.body = g.data[this.eventBarTextField]
                        } else if (this.eventBodyTemplate) {
                        tplData.body = this.eventBodyTemplate.apply(g.data)
                        }
                    f += this.eventTemplate.apply(tplData)
                    }
            }
        }
        m.css += ' sch-timetd';
        if (Ext.isIE) {
            m.attr += ' style="z-index:' + (cm.getColumnCount() - c) + '"'
        }
        return f
    },
    doRender: function(b, d, e, f, h, k) {
        ...
        rt = ts.row,
        last = h - 1;
        var l = 'width:' + this.getTotalWidth() + ';height:' + this.grid.getRowHeight() + 'px';
        var m = [],
        cb,
        c,
        p = {},
    ...
    resolveResource: function(t) {
        ...
        if (index >= 0) {
            a = this.ds.getAt(index)
            }
        return a
    },
    getScheduleRegion: function(a, b) {
        var c = a ? Ext.fly(this.getCell(this.ds.indexOf(a), this.leftColumns.length)).getRegion() : this.mainBody.getRegion(),
        taStart = this.timeAxis.getStart(),
        taEnd = this.timeAxis.getEnd(),
        dateConstraints = this.grid.getDateConstraints(a, b) || {
            start: taStart,
            end: taEnd
        },
        cm = this.cm,
        ...
        top = c.top + this.barMargin,
        bottom = (a ? (c.top + this.rowHeight) : c.bottom) - this.barMargin;
...
Ext.ns('Sch'); (function() {
    ...
    var n = Ext.ux.grid.LockingGridView.prototype;
    var o = {
        lockText: n.lockText,
        unlockText: n.unlockText,
        rowBorderWidth: n.rowBorderWidth,
        lockedBorderWidth: n.lockedBorderWidth,
        getEditorParent: n.getEditorParent,
        initElements: n.initElements,
        getLockedRows: n.getLockedRows,
        getLockedRow: n.getLockedRow,
        getCell: n.getCell,
        addRowClass: n.addRowClass,
        removeRowClass: n.removeRowClass,
        removeRow: n.removeRow,
        removeRows: n.removeRows,
        syncScroll: n.syncScroll,
        updateAllColumnWidths: n.updateAllColumnWidths,
        updateColumnWidth: n.updateColumnWidth,
        updateColumnHidden: n.updateColumnHidden,
        processRows: n.processRows,
        afterRenderUI: n.afterRenderUI,
        renderUI: n.renderUI,
        getOffsetWidth: n.getOffsetWidth,
        getResolvedXY: n.getResolvedXY,
        syncFocusEl: n.syncFocusEl,
        ensureVisible: n.ensureVisible,
        insertRows: n.insertRows,
        getColumnStyle: n.getColumnStyle,
        getLockedWidth: n.getLockedWidth,
        getTotalWidth: n.getTotalWidth,
        renderBody: n.renderBody,
        refreshRow: n.refreshRow,
        refresh: n.refresh,
        onDenyColumnLock: n.onDenyColumnLock,
        onColumnLock: n.onColumnLock,
        handleHdMenuClick: n.handleHdMenuClick,
        handleHdDown: n.handleHdDown,
        syncHeaderHeight: n.syncHeaderHeight,
        updateLockedWidth: n.updateLockedWidth,
        syncRowHeights: n.syncRowHeights,
        layout: n.layout,
        ...
        afterRender: function() {
            if (!this.forceFit && this.autoExpandTimeColumns && this.getScheduleSectionWidth() < this.getAvailableWidthForSchedule()) {
                this.fitColumns()
                }
            n.afterRender.apply(this, arguments)
            },
        ...
        getHeaderCell: function(a) {
            ...
            return Sch.AbstractSchedulerView.prototype.getHeaderCell.call(this, a - b)
            },
        scrollToTime: function(a, b) {
            var x = this.getXFromDate(a);
            if (x >= 0) {
        ...
        updateSortIcon: function(a, b) {
            var c = this.sortClasses,
            lockedHeaders = this.lockedHd.select('table:last-child td').removeClass(c),
            headers = this.mainHd.select('table:last-child td').removeClass(c),
            lockedLen = this.cm.getLockedCount(),
            cls = c[b == 'DESC' ? 1: 0];
            if (a < lockedLen) {
                lockedHeaders.item(a).addClass(cls)
                } else {
                headers.item(a - lockedLen).addClass(cls)
                }
        },
        getXFromDate: function(a) {
            var x = -1,
            tick = this.timeAxis.getTickFromDate(a);
            if (tick >= 0) {
                var b = Math.floor(tick),
                colWidth = this.getAccumulatedColumnWidth(this.leftColumns.length, this.leftColumns.length + b);
                x = colWidth + (b < this.timeAxis.getCount() ? this.cm.getColumnWidth(this.leftColumns.length + b) * (tick - b) : 0)
                }
            return x
        },
        renderHeaders: function() {
            ...
            var b = this.headerRows;
            if (b) {
                var c = this.cm.getLockedCount();
                if (b.top) {
                    var d = this.getHeaderRowCells('top'),
                    ...
                    if (c > 0) {
                        ...
                        d.splice(1, 0, staticCell)
                        }
                    unlockedHeaders += ts.header.apply({
                        tstyle: 'width:' + tw,
                        tclass: 'top',
                        cells: d.slice(1).join('')
                        })
                    }
                if (b.middle) {
                    var e = this.getHeaderRowCells('middle'),
                    staticCell = ts.gcell.apply({
                        cls: 'x-grid3-cell-first x-grid3-cell-last ux-grid-hd-nogroup-cell',
                        value: '&#160;',
                        row: 'middle',
                    ...
                    if (c > 0) {
                        ...
                        e.splice(1, 0, staticCell)
                        }
                    unlockedHeaders += ts.header.apply({
                        tstyle: 'width:' + tw,
                        tclass: 'middle',
                        cells: e.slice(1).join('')
                        })
                    }
            }
            unlockedHeaders += ts.header.apply({
        ...
        initTemplates: function() {
            var a = this.templates || {};
            if (!a.master) {
                a.master = this.masterTpl
            }
            if (!a.masterTpl) {
                a.masterTpl = this.masterTpl
            }
            this.templates = a;
            Sch.AbstractSchedulerView.prototype.initTemplates.call(this)
            },
        updateGroupStyles: function() {
            var a = this.lockedHd.query('.x-grid3-header-offset > table'),
            ...
            for (var b = 0; b < a.length; b++) {
                a[b].style.width = lockedWidth;
                unlockedTables[b].style.width = unlockedWidth;
                if (b < a.length - 1) {
                    var c = unlockedTables[b].className.match(this.rowPosRe)[1],
                    cells = unlockedTables[b].firstChild.firstChild.childNodes,
                    width;
                    cells[0].style.width = lockedWidth;
                    for (var i = 0; i < cells.length; i++) {
                        var d = headerRows[c][i + 1];
                        if (hasRightSectionColumns && i === cells.length - 1) {
                            cells[i].style.width = this.getRightGroupWidth() + 'px'
                        } else {
                            width = this.getHeaderGroupCellWidth(d.start, d.end, headerConfig[c].unit, headerConfig[c].increment || 1);
    ...
    Sch.AbstractLockingSchedulerView = Ext.extend(Sch.AbstractSchedulerView, Ext.apply({
        init: function(b) {
            if (!this.forceFit && this.autoExpandTimeColumns) {
                this.refresh = this.refresh.createInterceptor(function(a) {
        ...
        doRender: function(b, d, e, f, g, h) {
            ...
            for (var j = 0, len = d.length; j < len; j++) {
                ...
                for (var i = 0; i < g; i++) {
                    ...
                    p.attr = p.cellAttr = '';
                    p.value = c.renderer.call(c.scope || c, r.data[c.name], p, r, l, i, e, events);
                    p.style = c.style;
                ...
                if (r.dirty) {
                    m[1] = ' x-grid3-dirty-row'
                }
                rp.cols = g;
                if (this.getRowClass) {
                    m[2] = this.getRowClass(r, l, rp, e)
                    }
                rp.alt = m.join(' ');
                rp.cells = cb.join('');
                rp.tstyle = tstyle;
...
Sch.SchedulerGroupingView = Ext.extend(Sch.HorizontalSchedulerView, {
    ...
    init: function(a, b) {
        ...
        if (this.hideGroupedColumn) {
            ...
            colIndex = this.cm.findColumnIndex(c);
            this.lastGroupField = c;
        ...
        if (this.showSummaryInHeader) {
            this.grid.on('afterrender', function() {
                this.el.addClass('sch-groupingsummary')
                }, this)
            }
    },
    initTemplates: function() {
        ...
        if (!this.startGroup) {
            this.startGroup = new Ext.XTemplate('<div id="{groupId}" class="x-grid-group {cls}">', '<div id="{groupId}-hd" class="x-grid-group-hd" style="{style}"><div class="x-grid-group-title">', this.groupHeaderRenderer ? '{renderedHeader}': this.groupTextTpl, '</div></div>', '<div id="{groupId}-bd" class="x-grid-group-body">')
            }
    ...
    doRender: function(a, b, c, d, e, f) {
        ...
        var m = l.groupRenderer || l.renderer;
        var n = this.showGroupName ? (l.groupName || l.header) + ': ': '';
        var o = [],
        curGroup,
        i,
        len,
...
Ext.ns('Sch'); (function() {
    if (!Sch.LockingSchedulerView) {
        return
    }
    var u = Ext.grid.GroupingView.prototype;
    Sch.LockingGroupingSchedulerView = Ext.extend(Sch.LockingSchedulerView, {
        showSummaryInHeader: false,
        unlockedGroupHeaderRenderer: undefined,
        groupByText: u.groupByText,
        showGroupsText: u.showGroupsText,
        hideGroupedColumn: u.hideGroupedColumn,
        showGroupName: u.showGroupName,
        startCollapsed: u.startCollapsed,
        enableGrouping: u.enableGrouping,
        enableGroupingMenu: u.enableGroupingMenu,
        enableNoGroups: u.enableNoGroups,
        emptyGroupText: u.emptyGroupText,
        ignoreAdd: u.ignoreAdd,
        groupTextTpl: u.groupTextTpl,
        groupMode: Ext.grid.GroupingView.prototype.groupMode,
        gidSeed: u.gidSeed,
        constructor: function(a) {
            this.addEvents('togglegroup');
            Sch.LockingGroupingSchedulerView.superclass.constructor.call(this, a)
            },
        init: function(a, b) {
            Sch.LockingGroupingSchedulerView.superclass.init.apply(this, arguments);
            if (this.showSummaryInHeader) {
                this.grid.on('afterrender', function() {
                    this.el.addClass('sch-groupingsummary')
                    }, this)
        ...
        initTemplates: function() {
            ...
            if (!this.startLockedGroup) {
                this.startLockedGroup = new Ext.XTemplate('<div id="{groupId}" class="x-grid-group sch-grid-group-locked {cls}">', '<div id="{groupId}-hd" class="x-grid-group-hd" style="{lockedheaderstyle}"><div class="x-grid-group-title">' + (this.lockedGroupHeaderRenderer ? '{renderedHeader}': this.groupTextTpl), '</div></div>', '<div id="{groupId}-bd" class="x-grid-group-body">')
                }
            if (!this.startUnlockedGroup) {
                this.startUnlockedGroup = new Ext.XTemplate('<div id="{groupId}" class="x-grid-group x-grid-group-unlocked {cls}">' + '<div id="{groupId}-hd" class="x-grid-group-hd {unlockedheadercls}">' + '<div class="x-grid-group-title">' + (this.unlockedGroupHeaderRenderer ? '{renderedHeader}': this.groupTextTpl) + '</div>' + '</div>' + '<div id="{groupId}-bd" class="x-grid-group-body">')
                }
            this.startLockedGroup.compile();
            this.startUnlockedGroup.compile();
            this.endGroup = '</div></div>';
            Sch.LockingGroupingSchedulerView.superclass.initTemplates.call(this)
            },
        getLockedRows: function() {
            if (!this.enableGrouping) {
                return Sch.LockingGroupingSchedulerView.superclass.getLockedRows.call(this)
                }
            var r = [];
            var g,
            gs = this.getLockedGroups();
        ...
        doRender2: Sch.LockingSchedulerView.prototype.doRender,
        findGroup: u.findGroup,
        getGroups: u.getGroups,
        getLockedGroups: function() {
            return this.hasRows() ? this.lockedBody.dom.childNodes: []
            },
        onAdd: u.onAdd,
        onRemove: function(a, b, c, d) {
            Sch.LockingGroupingSchedulerView.superclass.onRemove.apply(this, arguments);
            var g = document.getElementById(b._groupId);
            if (g && g.childNodes[1].childNodes.length < 1) {
                this.el.select('[id="' + b._groupId + '"]').remove()
                }
            this.applyEmptyText()
            },
        refreshRow: function(a) {
            if (this.ds.getCount() == 1) {
                ...
                this.isUpdating = true;
                Sch.LockingGroupingSchedulerView.superclass.refreshRow.apply(this, arguments);
                this.isUpdating = false
            }
        },
        beforeMenuShow: u.beforeMenuShow,
        renderUI: function() {
            var a = Sch.LockingGroupingSchedulerView.superclass.renderUI.apply(this, arguments);
            if (this.interceptMouse) {
                this.mainBody.on('mousedown', this.interceptMouse, this);
                this.lockedBody.on('mousedown', this.interceptMouse, this)
                }
            return a
        },
        onGroupByClick: u.onGroupByClick,
        onShowGroupsClick: u.onShowGroupsClick,
        toggleGroup: function(a, b) {
            this.grid.stopEditing(true);
            a = Ext.getDom(a);
            var c = Ext.fly(a);
            b = b !== undefined ? b: c.hasClass('x-grid-group-collapsed');
            this.state[c.dom.id] = b;
            this.el.select('[id="' + c.dom.id + '"]')[b ? 'removeClass': 'addClass']('x-grid-group-collapsed');
            this.fireEvent('togglegroup', this, c.dom.id, b)
            },
        toggleAllGroups: u.toggleAllGroups,
        expandAllGroups: u.expandAllGroups,
        collapseAllGroups: u.collapseAllGroups,
        interceptMouse: u.interceptMouse,
        getGroup: u.getGroup,
        getGroupField: u.getGroupField,
        afterRender: function() {
            if (!this.ds || !this.cm) {
                return
            }
            Sch.LockingGroupingSchedulerView.superclass.afterRender.call(this);
        ...
        renderRows: function() {
            ...
            var b = !!a;
            if (this.hideGroupedColumn) {
                var c = this.cm.findColumnIndex(a);
                if (!b && this.lastGroupField !== undefined) {
                    this.mainBody.update('');
                    this.cm.setHidden(this.cm.findColumnIndex(this.lastGroupField), false);
                    delete this.lastGroupField
                } else if (b && this.lastGroupField === undefined) {
                    this.lastGroupField = a;
                    this.cm.setHidden(c, true)
                    } else if (b && this.lastGroupField !== undefined && a !== this.lastGroupField) {
                    this.mainBody.update('');
            ...
            return Sch.LockingGroupingSchedulerView.superclass.renderRows.apply(this, arguments)
            },
        processEvent: u.processEvent,
        doRender: function(a, b, c, d, e, f) {
            if (b.length < 1) {
                return ''
            }
            ...
            var j = 'width:' + this.getLockedWidth() + ';';
            var k = this.grid.getGridEl().id;
            var l = this.cm.config[colIndex];
            var m = l.groupRenderer || l.renderer;
            var n = this.showGroupName ? (l.groupName || l.header) + ': ': '';
            ...
            for (i = 0, len = o.length; i < len; i++) {
                g = o[i];
                this.doGroupStart(t, g, a, c, e);
                t[t.length] = this.doRender2.call(this, a, g.rs, c, g.startRow, e, f);
                this.doGroupEnd(t, g, a, c, e)
                }
        ...
        getGroupId: u.getGroupId,
        doGroupStart: function(a, g, b, c, d) {
            a[a.length] = [this.renderUnlockedHeader(g), this.renderLockedHeader(g)]
            },
        doGroupEnd: function(a, g, b, c, d) {
            a[a.length] = [this.endGroup, this.endGroup]
            },
        getRows: function() {
            if (!this.enableGrouping) {
                return Sch.LockingGroupingSchedulerView.superclass.getRows.call(this)
                }
            var r = [];
            var g,
            gs = this.getGroups();
        ...
        onColumnWidthUpdated: function(a, w, b) {
            Sch.LockingGroupingSchedulerView.superclass.onColumnWidthUpdated.call(this, a, w, b);
            this.updateGroupWidths()
            },
        onAllColumnWidthsUpdated: function(a, b) {
            Sch.LockingGroupingSchedulerView.superclass.onAllColumnWidthsUpdated.call(this, a, b);
            this.updateGroupWidths()
            },
        onColumnHiddenUpdated: function(a, b, c) {
            Sch.LockingGroupingSchedulerView.superclass.onColumnHiddenUpdated.call(this, a, b, c);
            this.updateGroupWidths()
            },
        onLayout: u.onLayout,
        onBeforeRowSelect: u.onBeforeRowSelect,
        constructId: u.constructId,
        canGroup: u.canGroup,
        getPrefix: u.getPrefix,
        getGroupEl: function(a) {
            return a.up('.x-grid-group')
            },
        getGroupHeaderEl: function(a) {
            return this.getGroupEl(a).down('.x-grid-group-hd')
            },
        getGroupEls: function(a) {
            return this.el.select('[id="' + this.getGroupEl(a).id + '"]')
            },
        getGroupElsById: function(a) {
            return this.el.select('[id="' + this.getPrefix(this.getGroupField()) + a + '"]')
            },
        renderUnlockedHeader: function(a) {
            if (this.unlockedGroupHeaderRenderer) {
                a.renderedHeader = this.unlockedGroupHeaderRenderer.call(this, a)
                }
            return this.startUnlockedGroup.apply(a)
            },
        renderLockedHeader: function(a) {
            if (this.lockedGroupHeaderRenderer) {
                a.renderedHeader = this.lockedGroupHeaderRenderer.call(this, a)
                }
            return this.startLockedGroup.apply(a)
            }
    })
    })();
Ext.ns('Sch');
Sch.ColumnFactory = {
    ...
    createHeaderRows: function(a, b) {
        var c = {};
        if (b.top) {
            var d;
            if (b.top.cellGenerator) {
                d = b.top.cellGenerator.call(this, a.getStart(), a.getEnd())
                } else {
                d = this.createHeaderRow(a, b.top)
                }
            c.top = d
        }
        if (b.bottom) {
            var e;
            if (b.middle.cellGenerator) {
                e = b.middle.cellGenerator.call(this, a.getStart(), a.getEnd())
                } else {
                e = this.createHeaderRow(a, b.middle)
                }
            c.middle = e
        }
        return c
    },
    createHeaderRow: function(a, b) {
        var c = [],
        colConfig,
        start = a.getStart(),
        end = a.getEnd(),
        ...
        intervalEnd;
        while (dt < end) {
            intervalEnd = Sch.util.Date.min(a.getNext(dt, b.unit, b.increment || 1), end);
            colConfig = {
                align: align,
                start: dt,
                end: intervalEnd,
            ...
            if (b.renderer) {
                colConfig.header = b.renderer.call(b.scope || this, dt, intervalEnd, colConfig, i)
                } else {
                colConfig.header = this.defaultRenderer(dt, intervalEnd, b.dateFormat, colConfig, i)
                }
            c.push(colConfig);
            dt = intervalEnd;
            i++
...
Ext.extend(Sch.EventSelectionModel, Ext.grid.AbstractSelectionModel, {
    ...
    onEventClick: function(g, a, e) {
        var b = e.getTarget(this.grid.eventSelector);
...
Ext.extend(Sch.LazyResizable, Ext.Resizable, {
    ...
    resizeElement: Ext.Resizable.prototype.resizeElement.createInterceptor(function() {
        ...
        if (a.width !== oldWidth) {
            this.fireEvent('partialresize', this, a.width, oldWidth)
            }
    })
    });
Ext.ns('Sch.plugins');
Sch.plugins.DragSelector = Ext.extend(Ext.dd.DragTracker, {
    constructor: function(a) {
        ...
        Sch.plugins.DragSelector.superclass.constructor.call(this, a)
        },
    init: function(a) {
        a.on({
            afterrender: this.onSchedulerRender,
    ...
    onStart: function(e) {
        ...
        var c = [];
        view.mainBody.select(b.eventSelector).each(function(a) {
            c[c.length] = {
                region: a.getRegion(),
                node: a.dom
            }
...
Sch.plugins.EventEditor = Ext.extend(Ext.form.FormPanel, {
    ...
    show: function(a, b) {
        ...
        this.eventRecord = a;
        var c = Sch.util.Date.getDurationInHours(a.get('StartDate'), a.get('EndDate'));
    ...
    onSaveClick: function() {
        ...
        if (form.isValid() && this.fireEvent('beforeeventsave', this, record) !== false) {
            ...
            if (b && hours >= 0) {
                end = Sch.util.Date.add(b, Sch.util.Date.MINUTE, hours * 60)
                } else {
                return
            }
    ...
    onMouseDown: function(e) {
        ...
        if (!this.collapsed && !e.within(this.getEl()) && (!a.tf.view || !e.within(a.tf.view.el)) && (!a.df.menu || !e.within(a.df.menu.el))) {
            this.collapse(false)
            }
    },
    onSchedulerDestroy: function() {
...
Sch.AbstractTimeSpan = Ext.extend(Ext.util.Observable, {
    ...
    onAfterRender: function(a) {
        ...
        a.mon(a.getColumnModel(), 'hiddenchange', this.renderElements, this)
        },
    renderElements: function() {
        if (this.disabled || !this.scheduler.viewReady)
            return;
...
Sch.plugins.Lines = Ext.extend(Sch.AbstractTimeSpan, {
    ...
    init: function(a) {
        if (!this.template) {
            this.template = new Ext.XTemplate('<tpl for=".">', '<div title="{[values.Date.format("' + this.dateFormat + '")]} - {[values.Text || ""]}" class="sch-timeline {uniquecls} {Cls}" style="left:{left}px;top:{top}px;height:{height}px;width:{width}px"></div>', '</tpl>')
            }
        Sch.plugins.Lines.superclass.init.call(this, a)
        },
    getElementData: function(a, b) {
        var s = this.store,
...
Sch.plugins.Zones = Ext.extend(Sch.AbstractTimeSpan, {
    init: function(a) {
        ...
        Sch.plugins.Zones.superclass.init.call(this, a)
        },
    getElementData: function(a, b) {
        var s = this.store,
        scheduler = this.scheduler,
        ...
        for (var i = 0, l = s.getCount(); i < l; i++) {
            ...
            if (Sch.util.Date.intersectSpans(spanStart, spanEnd, a, b)) {
                region = view.getTimeSpanRegion(Sch.util.Date.max(spanStart, a), Sch.util.Date.min(spanEnd, b));
                data[data.length] = Ext.apply({
                    left: region.left,
                    top: region.top,
...
Ext.ns('Sch.plugins');
Sch.plugins.SummaryColumn = Ext.extend(Ext.grid.Column, {
    showPercent: false,
    nbrDecimals: 1,
    dayText: 'd',
    hourText: 'h',
    ...
    constructor: function(a) {
        ...
        Sch.plugins.SummaryColumn.superclass.constructor.call(this, a)
        },
    init: function(a) {
        this.grid = a;
    ...
    renderer: function(v, p, a) {
        ...
        viewEnd = g.getEnd(),
        retVal = 0,
        totalAllocatedMinutesInView = this.calculate(s.query('ResourceId', a.get('Id')), viewStart, viewEnd);
        if (totalAllocatedMinutesInView <= 0) {
            return ''
        }
        if (this.showPercent) {
            var b = Sch.util.Date.getDurationInMinutes(viewStart, viewEnd);
            return Math.min(100, (Math.round((totalAllocatedMinutesInView * 100) / b))) + ' %'
        } else {
    ...
    calculate: function(b, c, d) {
        var e = 0,
        eventStart,
        eventEnd;
        b.each(function(a) {
            eventStart = a.get('StartDate');
            eventEnd = a.get('EndDate');
            if (Sch.util.Date.intersectSpans(c, d, eventStart, eventEnd)) {
                e += Sch.util.Date.getDurationInMinutes(Sch.util.Date.max(eventStart, c), Sch.util.Date.min(eventEnd, d))
                }
        });
        return e
    }
...
Sch.SchedulerPanel = Ext.extend(Sch.AbstractSchedulerPanel, {
    ...
    onDestroy: function() {
        if (this.eventStore.autoDestroy) {
    ...
    afterRender: function() {
        ...
        this.el.addClass('sch-' + this.viewOrientation);
    ...
    applyStartEndParameters: function(a, b) {
        b.params = b.params || {};
        b.params[this.startParamName] = this.getStart();
        b.params[this.endParamName] = this.getEnd()
        },
    getTooltipRecord: function(a) {
        return this.getEventRecordFromDomId(a.id)
        },
    ...
    configureFunctionality: function() {
        ...
        if (this.resizeHandles !== 'none' && Sch.Resize) {
            this.resizePlug = new Sch.Resize(this, Ext.applyIf({
                validatorFn: function(a, b, c, d) {
                    return (this.allowOverlap || this.isDateRangeAvailable(c, d, b.id, a.id)) && this.resizeValidatorFn.apply(f, arguments)
                    },
        ...
        if (this.enableEventDragDrop !== false && Sch.DragDrop) {
            this.dragdropPlug = new Sch.DragDrop(this, {
                validatorFn: function(a, b, c, d, e) {
                    return (this.allowOverlap || this.isDateRangeAvailable(c, Sch.util.Date.add(c, Sch.util.Date.MILLI, d), a[0].id, b.id)) && this.dndValidatorFn.apply(f, arguments)
                    },
                validatorFnScope: this,
        ...
        if (this.enableDragCreation !== false && Sch.DragCreator) {
            this.dragCreator = new Sch.DragCreator(this, Ext.applyIf({
                disabled: this.readOnly,
                validatorFn: function(a, b, c) {
                    return (this.allowOverlap || this.isDateRangeAvailable(b, c, null, a.id)) && this.createValidatorFn.apply(f, arguments)
                    },
                validatorFnScope: this
            }, this.createConfig || {}))
            }
    ...
    onBeforeDragCreate: function(s, e) {
    ...
    getResourceByEventRecord: function(a) {
        return this.resourceStore.getById(a.get('ResourceId'))
        },
    isDateRangeAvailable: function(a, b, c, d) {
        var e = true;
        this.eventStore.each(function(r) {
            if (Sch.util.Date.intersectSpans(a, b, r.get('StartDate'), r.get('EndDate')) && (d === r.get('ResourceId') && (!c || c !== r.id))) {
                e = false;
                return false
            }
        });
    ...
    getEventsInView: function() {
        ...
        return this.eventStore.queryBy(function(a) {
            ...
            eventEnd = a.get('EndDate');
            return Sch.util.Date.intersectSpans(b, eventEnd, c, viewEnd)
            })
        },
    constructor: function(a) {
        this.addEvents('eventclick', 'eventdblclick', 'eventcontextmenu', 'beforeresize', 'resizestart', 'partialresize', 'afterresize', 'beforednd', 'dndstart', 'drop', 'afterdnd', 'beforedragcreate', 'dragcreatestart', 'dragcreateend', 'afterdragcreate', 'beforeeventadd');
    ...
    getDateConstraints: function(a, b) {},
    getRowHeight: function() {
        return this.view.rowHeight
    },
    setRowHeight: function(a) {
        this.getView().setRowHeight(a)
        }
});
...
Ext.ns('Sch.plugins');
Sch.plugins.TimeGap = Ext.extend(Sch.plugins.Zones, {
    ...
    init: function(a) {
        ...
        Sch.plugins.TimeGap.superclass.init.apply(this, arguments)
        },
    populateStore: function(c) {
        var d = this.scheduler.getEventsInView(),
        timeGaps = [],
        ...
        index = 0,
        r;
        d.sort('ASC', function(a, b) {
            return a.get('StartDate') - b.get('StartDate')
            });
        r = d.itemAt(0);
        while (cursor < viewEnd && index < l) {
            eventStart = r.get('StartDate');
            if (!Sch.util.Date.betweenLesser(cursor, eventStart, r.get('EndDate')) && cursor < eventStart) {
                timeGaps.push(new this.store.recordType({
                    StartDate: cursor,
                    EndDate: eventStart,
...
Ext.ns('Sch.plugins');
Sch.plugins.CurrentTimeLine = function(a, b) {
    Ext.apply(this, a);
    this.grid = b;
...
Ext.extend(Sch.plugins.CurrentTimeLine, Sch.plugins.Lines, {
    ...
    init: function(a) {
        ...
        if (this.autoUpdate) {
            ...
            this.runner.start({
                run: function() {
                    b.getAt(0).set('Date', new Date())
                    },
                interval: this.updateInterval
            })
            }
        a.on('destroy', this.onHostDestroy, this);
        this.store = b;
...
Sch.ClockTemplate = function() {
    ...
    hourTop = 3,
    hourLeft = 10;
    function getHourStyleIE(a) {
        var b = a * c,
        cosV = cos(b),
        sinV = sin(b),
        y = hourHeight * sin((90 - a) * c),
        x = hourHeight * cos((90 - a) * c),
        topAdjust = Math.min(hourHeight, hourHeight - y),
        leftAdjust = a > 180 ? x: 0,
    ...
    function getMinuteStyleIE(a) {
        var b = a * c,
        cosV = cos(b),
        sinV = sin(b),
        y = minuteHeight * sin((90 - a) * c),
        x = minuteHeight * cos((90 - a) * c),
    ...
    function getStyle(a) {
        return String.format("transform:rotate({0}deg);-moz-transform: rotate({0}deg);-webkit-transform: rotate({0}deg);-o-transform:rotate({0}deg);", a)
        }
...
Ext.ns('Sch');
Sch.DragCreator = function(a, b) {
    Ext.apply(this, b || {});
    this.lastTime = new Date(1970);
    this.scheduler = a;
    this.scheduler.on({
    ...
    Sch.DragCreator.superclass.constructor.call(this)
    };
Ext.extend(Sch.DragCreator, Ext.util.Observable, {
    disabled: false,
    showHoverTip: true,
    showDragTip: true,
    dragTolerance: 2,
    template: new Ext.Template('<div class="sch-dragcreator-proxy sch-event">' + '<div class="sch-event-inner"></div>' + '</div>', {
        compiled: true,
        disableFormats: true
    }),
    ...
    getProxy: function() {
        if (!this.proxy) {
            this.proxy = this.template.append(this.scheduler.getView().el, {}, true)
            }
        return this.proxy
    },
    onMouseMove: function(e) {
        ...
        if (e.getTarget('.sch-timetd', 2) || e.getTarget('.x-grid3-row', 1)) {
            var b = this.scheduler.getDateFromDomEvent(e, 'floor');
            if (b) {
                if (b - this.lastTime !== 0) {
                    ...
                    if (a.hidden) {
                        try {
                            a[Sch.util.Date.compareUnits(this.scheduler.getTimeResolution().unit, Sch.util.Date.DAY) >= 0 ? 'addClass': 'removeClass']('sch-day-resolution');
                            a.show()
                            } catch(e) {}
                    }
                }
    ...
    updateHoverTip: function(a) {
        if (a) {
            var b = this.scheduler.getFormattedDate(a);
    ...
    onBeforeDragStart: function(e) {
        var t = e.getTarget('.sch-timetd', 2),
        s = this.scheduler,
        resourceRecord = s.view.resolveResource(t);
        if (!this.disabled && t && s.fireEvent('beforedragcreate', s, resourceRecord, e) !== false) {
            this.resourceRecord = resourceRecord;
            this.originalStart = s.getDateFromDomEvent(e);
            this.resourceRegion = s.view.getScheduleRegion(this.resourceRecord, this.originalStart);
    ...
    onDragStart: function() {
        var a = this,
        view = a.scheduler.view,
        proxy = a.getProxy();
        a.containerRegion = view.mainBody.getRegion();
        this.dragging = true;
        if (a.scheduler.snapToIncrement) {
            a.originalStart = a.scheduler.floorDate(a.originalStart);
            var b = a.scheduler.getXYFromDate(a.originalStart);
            a.tracker.startXY[0] = b[0] + a.containerRegion.left;
            a.tracker.startXY[1] = b[1] + a.containerRegion.top
        }
        if (this.hoverTip) {
        ...
        a.end = a.start;
        a.dragRegion.top = a.resourceRegion.top;
        a.dragRegion.bottom = a.resourceRegion.bottom;
        proxy.show();
        proxy.setRegion({
            top: a.dragRegion.top,
            right: a.tracker.startXY[0],
            bottom: a.dragRegion.bottom,
            left: a.tracker.startXY[0]
            });
        a.scheduler.fireEvent('dragcreatestart', a.scheduler);
        if (a.showDragTip) {
            a.dragTip.update(a.start, a.end, true);
            a.dragTip.targetXY = proxy.getXY();
            a.dragTip.enable().show()
            }
    },
    onDrag: function(e) {
        var a = this,
        s = a.scheduler,
        rawDate = s.getDateFromDomEvent(e);
        if (!rawDate) {
            return
        }
        var b = a.tracker.startXY,
        xy = a.tracker.getXY(),
        x = xy[0],
        view = s.view,
        w = Math.abs(b[0] - xy[0]),
        dr = a.dragRegion;
        if (rawDate) {
            if (rawDate > a.originalStart) {
                a.start = s.floorDate(a.originalStart);
                a.end = s.roundDate(rawDate);
                if (a.dateConstraints) {
                    a.end = Sch.util.Date.constrain(a.end, a.dateConstraints.start, a.dateConstraints.end)
                    }
            } else {
                a.end = s.roundDate(a.originalStart);
                a.start = s.roundDate(rawDate);
                if (a.dateConstraints) {
                    a.start = Sch.util.Date.constrain(a.start, a.dateConstraints.start, a.dateConstraints.end)
                    }
            }
            this.valid = this.validatorFn.call(this.validatorFnScope || this, this.resourceRecord, a.start, a.end) !== false;
            if (a.showDragTip) {
                a.dragTip.update(a.start, a.end, this.valid)
                }
            if (rawDate > a.originalStart) {
                dr.left = b[0];
                if (s.snapToIncrement) {
                    dr.right = s.getXYFromDate(a.end)[0] + a.containerRegion.left
                } else {
                    dr.right = b[0] + w;
                    dr.constrainTo(this.resourceRegion)
                    }
            } else {
                if (s.snapToIncrement) {
                    dr.left = s.getXYFromDate(a.start)[0] + a.containerRegion.left
                } else {
                    dr.left = b[0] - w;
                    dr.constrainTo(this.resourceRegion)
                    }
    ...
    onDragEnd: function(e) {
        ...
        if (this.valid) {
            var a = new s.eventStore.recordType({
                ResourceId: this.resourceRecord.get('Id'),
                StartDate: this.start,
                EndDate: this.end
            });
            s.fireEvent('dragcreateend', s, a, this.resourceRecord, e)
            } else {
            this.proxy.hide()
            }
        this.scheduler.fireEvent('afterdragcreate', s);
        if (this.hoverTip) {
    ...
    onSchedulerRender: function(a) {
        ...
        if (this.showHoverTip) {
            this.hoverTipTemplate = this.hoverTipTemplate || new Sch.ClockTemplate();
            this.hoverTip = new Ext.ToolTip(Ext.applyIf({
                renderTo: document.body,
                target: b,
                disabled: this.disabled,
                delayShow: Ext.emptyFn
            }, this.tipCfg));
            this.hoverTip.on('beforeshow', this.onBeforeShow, this);
            a.mon(b, 'mousemove', this.onMouseMove, this);
            a.mon(b, 'mouseleave', function() {
                this.hoverTip.hide()
                }, this)
            }
    },
    ...
    onBeforeShow: function(a) {
        return ! this.disabled && !this.dragging && a.body.dom.innerHTML !== "" && this.lastTime !== null
    }
});
Ext.ns('Sch');
Sch.DragDrop = Ext.extend(Ext.util.Observable, {
    validatorFn: function(a, b, c, d, e) {
        return true
    },
    enableCopy: false,
    ...
    constructor: function(a, b) {
        ...
        Sch.DragDrop.superclass.constructor.call(this)
        },
    cleanUp: function() {
        if (this.scheduler.dragZone) {
            this.scheduler.dragZone.destroy()
            }
        if (this.scheduler.dropZone) {
            this.scheduler.dropZone.destroy()
            }
        if (this.tip) {
            this.tip.destroy()
            }
    ...
    onRender: function() {
        ...
        if (!this.dragConfig.useDragProxy && a) {
    ...
    initProxyLessDD: function() {
        var s = this.scheduler,
        v = s.getView();
        s.dragZone = new Sch.PointSchedulerDragZone(v.mainBody, Ext.apply({
            ddGroup: s.id,
            scheduler: s,
    ...
    initProxyDD: function() {
        var s = this.scheduler,
        v = this.scheduler.getView();
        s.dragZone = new Sch.SchedulerDragZone(v.scroller, Ext.apply({
            ddGroup: this.scheduler.id,
            scheduler: this.scheduler,
            enableCopy: this.enableCopy
        }, this.dragConfig));
        s.dropZone = new Sch.SchedulerDropZone(v.mainBody, Ext.apply({
            ddGroup: this.scheduler.id,
            scheduler: this.scheduler,
            enableCopy: this.enableCopy,
            validatorFn: this.validatorFn,
...
Sch.ClassicDDProxy = function(c) {
    return new Ext.XTemplate('<span class="sch-dd-newtime">' + '{[ this.getText(values) ]}' + '</span>', {
        getText: function(a) {
            var b = c.getFormattedDate(a.StartDate);
            if (a.Duration) {
                b += ' - ' + c.getFormattedEndDate(Sch.util.Date.add(a.StartDate, Sch.util.Date.MILLI, a.Duration), a.StartDate)
                }
            return b
        }
    })
    };
Sch.SchedulerDragZone = Ext.extend(Ext.dd.DragZone, {
    containerScroll: true,
    onStartDrag: function() {
        var s = this.scheduler;
        s.fireEvent('dndstart', s, this.dragData.records)
        },
    getDragData: function(e) {
        var s = this.scheduler,
        sourceNode = e.getTarget(s.eventSelector);
        if (sourceNode) {
            var a = s.getSelectionModel(),
            ...
            if (!a.isSelected(eventEl)) {
                a.select(eventEl, a.multiSelect, true)
                }
            if (s.fireEvent('beforednd', s, sourceEventRecord, e) === false) {
                return null
            }
            var b,
            start = sourceEventRecord.get('StartDate'),
            selNodes = a.getSelectedNodes(),
            copy,
            wrap = Ext.get(Ext.DomHelper.createDom({
                cls: 'sch-dd-wrap',
                children: [{
                    cls: 'sch-dd-proxy-hd',
                    html: '&nbsp'
            ...
            for (var i = 0, len = selNodes.length; i < len; i++) {
                copy = selNodes[i].cloneNode(true);
                copy.id = Ext.id();
            ...
            return {
                ...
                records: s.getSelectedRecords(),
                duration: sourceEventRecord.get('EndDate') - start
            }
        }
        return null
    },
...
Sch.SchedulerDropZone = Ext.extend(Ext.dd.DropZone, {
    constructor: function() {
        Sch.SchedulerDropZone.superclass.constructor.apply(this, arguments);
        this.proxyTpl = this.proxyTpl || new Sch.ClassicDDProxy(this.scheduler)
        },
    validatorFn: Ext.emptyFn,
    getTargetFromEvent: function(e) {
        return e.getTarget('.sch-timetd')
        },
    onNodeEnter: function(a, b, e, c) {
        Ext.fly(a).addClass('sch-dd-cellover')
        },
    onNodeOut: function(a, b, e, c) {
        Ext.fly(a).removeClass('sch-dd-cellover')
        },
    onNodeOver: function(a, b, e, c) {
        ...
        this.proxyTpl.overwrite(b.proxy.el.child('.sch-dd-proxy-hd'), {
            StartDate: date,
            Duration: c.duration
        });
        var d = s.view.resolveResource(a);
        if (this.validatorFn.call(this.validatorFnScope || this, c.records, d, date, c.duration, e) !== false) {
    ...
    onNodeDrop: function(a, b, e, c) {
        var s = this.scheduler,
        v = s.getView(),
        targetRecord = s.view.resolveResource(a),
        date = s.getDateFromDomEvent(e, 'round'),
        valid = false,
        isCopy = this.enableCopy && e.ctrlKey;
        if (date && this.validatorFn.call(this.validatorFnScope || this, c.records, targetRecord, date, c.duration, e) !== false) {
            var d,
            index = s.resourceStore.indexOf(targetRecord);
            if (isCopy) {
                d = this.copyRecords(c.records, date, targetRecord, c.sourceEventRecord, index);
                valid = true
            } else {
                valid = this.updateRecords(c.records, date, targetRecord, c.sourceEventRecord, index, c)
                }
            if (valid) {
                s.getSelectionModel().clearSelections()
                }
            s.fireEvent('drop', s, isCopy ? d: c.records, isCopy)
            }
        s.fireEvent('afterdnd', s);
        return valid
    },
    updateRecords: function(a, b, c, d, e, f) {
        if (a.length === 1) {
            d.beginEdit();
            d.set('ResourceId', c.get('Id'));
            d.set('StartDate', b);
            d.set('EndDate', Sch.util.Date.add(b, Sch.util.Date.MILLI, f.duration));
            d.endEdit();
            return true
        }
        var g = d.get('StartDate'),
        resourceStore = c.store,
        diffStart = b - g,
        sourceIndex = resourceStore.indexOfId(d.get('ResourceId')),
        diff,
        oldIndex,
        newResourceRecord,
        r,
        ...
        for (i = 0; i < a.length; i++) {
            r = a[i];
            oldIndex = resourceStore.indexOfId(r.get('ResourceId'));
            newIndex = oldIndex - sourceIndex + e;
        ...
        for (i = 0; i < a.length; i++) {
            ...
            newResourceRecord = resourceStore.getAt(e + diff);
            r.beginEdit();
            r.set('ResourceId', newResourceRecord.get('Id'));
            r.set('StartDate', Sch.util.Date.add(r.get('StartDate'), Sch.util.Date.MILLI, diffStart));
            r.set('EndDate', Sch.util.Date.add(r.get('EndDate'), Sch.util.Date.MILLI, diffStart));
            r.endEdit()
            }
        return true
    },
    copyRecords: function(a, b, c, d, e) {
        var f = a[0],
        newItem = f.copy(),
        duration = d.get('EndDate') - d.get('StartDate');
        newItem.set('ResourceId', c.get('Id'));
        newItem.set('StartDate', b);
        newItem.set('EndDate', Sch.util.Date.add(b, Sch.util.Date.MILLI, duration));
        return [newItem]
        }
});
Sch.SchedulerDragProxy = Ext.extend(Ext.dd.StatusProxy, {
    constructor: function(a) {
        Ext.apply(this, a);
...
Sch.PointSchedulerDragZone = Ext.extend(Ext.dd.DragZone, {
    ...
    resolveStartEndDates: function(a) {
        ...
        if (!b.startsOutsideView) {
            ...
            end = Sch.util.Date.add(start, Sch.util.Date.MILLI, b.duration)
            } else if (!b.endsOutsideView) {
            end = this.scheduler.getDateFromXY([a.left + b.eventNodeWidth, a.top], 'round') || b.end;
            start = Sch.util.Date.add(end, Sch.util.Date.MILLI, -b.duration)
            }
        b.start = start;
        b.end = end
    },
    ...
    onStartDrag: function() {
        ...
        if (this.showTooltip) {
            ...
            this.tip.enable();
            this.tip.mouseOffset = [0, -dd.offsets[1] + 4];
    ...
    getDragData: function(e) {
        ...
        if (s.fireEvent('beforednd', s, a, e) === false) {
            return null
        }
        var b = e.getXY(),
        eventEl = Ext.get(t),
        ...
        if (s.constrainDragToResource) {
            this.constrainTo(this.view.getScheduleRegion(resource, a), eventRegion);
            this.setYConstraint(eventRegion.top, eventRegion.top)
            } else {
            this.constrainTo(this.view.getScheduleRegion(null, a), eventRegion)
            }
        if (tickSize !== 1) {
    ...
    onDragDrop: function(e, a) {
        ...
        if (this.valid && dragData.start && dragData.end) {
            ...
            modified = (dragData.start - dragData.origStart) !== 0 || (dragData.end - dragData.origEnd) !== 0 || newResId !== b.get('ResourceId');
            b.beginEdit();
            b.set('ResourceId', dragData.resourceRecord.get('Id'));
            b.set('StartDate', dragData.start);
            b.set('EndDate', dragData.end);
            b.endEdit();
            s.fireEvent('drop', s, [b], false)
            }
        if (this.tip) {
            this.tip.disable()
            }
        if (this.valid && modified) {
            this.onValidDrop(target, e, a)
            } else {
            this.onInvalidDrop(target, e, a)
            }
        s.fireEvent('afterdnd', s)
        },
    ...
    resolveResource: function(a) {
        ...
        var c = document.elementFromPoint(a[0] - this.dragData.bodyScroll.left, a[1] - this.dragData.bodyScroll.top);
...
Sch.Resize = function(a, b) {
    ...
    Sch.Resize.superclass.constructor.call(this)
    };
Ext.extend(Sch.Resize, Ext.util.Observable, {
    useTooltip: true,
    validatorFn: Ext.emptyFn,
    validatorFnScope: null,
    onSchedulerRender: function() {
        var s = this.scheduler;
        s.mon(s.getView().mainBody, 'mousedown', this.onMouseDown, this, {
    ...
    onMouseDown: function(e) {
        ...
        rec = s.getEventRecordFromElement(domEl);
    ...
    createResizable: function(a, b, e) {
        ...
        s = this.scheduler,
        v = s.getView(),
        isStart = !!t.className.match('sch-resizable-handle-start'),
        resourceRecord = s.getResourceByEventRecord(b),
        widthIncrement = v.getSnapPixelAmount(),
        startLeft = a.getLeft(),
        startRight = a.getRight(),
        currentWidth = a.getWidth(),
        resourceRegion = v.getScheduleRegion(resourceRecord, b),
        dateConstraints = s.getDateConstraints(resourceRecord, b);
        this.resizable = new Sch.LazyResizable(a, {
            startLeft: startLeft,
            ...
            listeners: {
                partialresize: {
                    fn: this[isStart ? 'partialWestResize': 'partialEastResize'],
                    scope: this
                },
                resize: {
                    fn: this.afterResize,
        ...
        if (this.useTooltip) {
            ...
            var c = b.get('StartDate'),
            end = b.get('EndDate');
            this.tip.update(c, end, true);
            this.tip.targetXY = [e.getPageX(), a.getY()];
            this.tip.enable().show()
            }
        if (Ext.isIE) {
            a.up('.x-grid3-cell-inner').setStyle('z-index', 100)
            }
    },
    partialEastResize: function(r, a, b) {
        var s = this.scheduler,
        end = s.getDateFromX(r.startLeft + Math.min(a, this.resizable.maxWidth), 'round');
        if (!end) {
            return
        }
        ...
        var c = r.eventRecord.get('StartDate'),
        valid = this.validatorFn.call(this.validatorFnScope || this, r.resourceRecord, r.eventRecord, c, end) !== false;
        r.end = end;
        s.fireEvent('partialresize', s, r.eventRecord, c, end, r.el);
        if (this.useTooltip) {
            this.tip.update(c, end, valid)
            }
    },
    partialWestResize: function(r, a, b) {
        var s = this.scheduler,
        start = s.getDateFromX(r.startRight - a, 'round');
        if (!start) {
            return
        }
        if (r.dateConstraints) {
        ...
        var c = r.eventRecord.get('EndDate'),
        valid = this.validatorFn.call(this.validatorFnScope || this, r.resourceRecord, r.eventRecord, start, c) !== false;
        r.start = start;
        s.fireEvent('partialresize', s, r.eventRecord, start, c, r.el);
        if (this.useTooltip) {
            this.tip.update(start, c, valid)
            }
    },
    afterResize: function(r, w, h, e) {
        ...
        if (start && end && (end - start > 0) && ((start - oldStart !== 0) || (end - oldEnd !== 0)) && this.validatorFn.call(this.validatorFnScope || this, a, eventRecord, start, end, e) !== false) {
            ...
            scheduler.getView().refreshRow(a)
            }
        r.destroy();
...
Sch.TimeAxis = Ext.extend(Ext.util.Observable, {
    ...
    reconfigure: function(a) {
        ...
        this.tickStore.loadData(b)
        },
    setTimeSpan: function(a, b) {
        var c = this.generateTicks(a, b);
        this.tickStore.loadData(c)
        },
    getIncrement: function() {
        return this.increment
    },
    filterBy: function(b, c) {
        this.continuous = false;
        this.tickStore.filterBy(function(t, a) {
            return b.call(c || this, t.data, a)
            }, this)
        },
    ...
    generateTicks: function(a, b, c, d) {
        var e = [],
        intervalEnd;
        c = c || this.unit;
        d = d || this.increment;
        if (this.autoAdjust) {
            ...
            b = this.ceilDate(b || Sch.util.Date.add(a, this.mainUnit, this.defaultSpan), false)
            }
        while (a < b) {
            intervalEnd = this.getNext(a, c, d);
            e.push({
                start: a,
                end: intervalEnd
            });
    ...
    getTickFromDate: function(a) {
        if (this.getStart() > a || this.getEnd() < a) {
            return - 1
        }
        var b = this.tickStore.getRange(),
        tickEnd;
        for (var i = 0, l = b.length; i < l; i++) {
            tickEnd = b[i].data.end;
            if (a <= tickEnd) {
                var c = b[i].data.start;
                return i + (a > c ? (a - c) / (tickEnd - c) : 0)
                }
        }
        return - 1
    },
    getDateFromTick: function(a) {
        var b = this.tickStore.getCount();
        if (a === b) {
            return this.getEnd()
            }
        var t = this.getAt(Math.floor(a));
        return Sch.util.Date.add(t.start, Sch.util.Date.MILLI, (a - Math.floor(a)) * (t.end - t.start))
        },
    getAt: function(a) {
    ...
    getTicks: function() {
        var a = [];
        this.tickStore.each(function(r) {
            a.push(r.data)
            });
        return a
    },
    ...
    roundDate: function(a) {
        ...
        case Sch.util.Date.MILLI:
            var c = Sch.util.Date.getDurationInMilliseconds(relativeTo, b),
            snappedMilliseconds = Math.round(c / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.MILLI, snappedMilliseconds);
            break;
        case Sch.util.Date.SECOND:
            var d = Sch.util.Date.getDurationInSeconds(relativeTo, b),
            snappedSeconds = Math.round(d / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.MILLI, snappedSeconds * 1000);
            break;
        case Sch.util.Date.MINUTE:
            var e = Sch.util.Date.getDurationInMinutes(relativeTo, b),
            snappedMinutes = Math.round(e / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.SECOND, snappedMinutes * 60);
            break;
        case Sch.util.Date.HOUR:
            var f = Sch.util.Date.getDurationInHours(this.getStart(), b),
            snappedHours = Math.round(f / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.MINUTE, snappedHours * 60);
            break;
        case Sch.util.Date.DAY:
            var g = Sch.util.Date.getDurationInDays(relativeTo, b),
            snappedDays = Math.round(g / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.HOUR, snappedDays * 24);
            break;
        case Sch.util.Date.WEEK:
            b.clearTime();
            var h = b.getDay() - this.weekStartDay,
            toAdd;
            if (h < 0) {
                h = 7 + h
            }
            ...
            b = Sch.util.Date.add(b, Sch.util.Date.DAY, toAdd);
            break;
        case Sch.util.Date.MONTH:
            var i = Sch.util.Date.getDurationInMonths(relativeTo, b) + (b.getDate() / b.getDaysInMonth()),
            snappedMonths = Math.round(i / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.MONTH, snappedMonths);
            break;
        case Sch.util.Date.QUARTER:
            b.clearTime();
            b.setDate(1);
            b = Sch.util.Date.add(b, Sch.util.Date.MONTH, 3 - (b.getMonth() % 3));
            break;
        case Sch.util.Date.YEAR:
            var j = Sch.util.Date.getDurationInYears(relativeTo, b),
            snappedYears = Math.round(j / increment) * increment;
            b = Sch.util.Date.add(relativeTo, Sch.util.Date.YEAR, snappedYears);
            break
        }
        return b
    },
    floorDate: function(a, b) {
        b = b !== false;
        var c = a.clone(),
        relativeTo = b ? this.getStart() : null,
        increment = this.resolutionIncrement;
        switch (b ? this.resolutionUnit: this.mainUnit) {
        case Sch.util.Date.MILLI:
            if (b) {
                var d = Sch.util.Date.getDurationInMilliseconds(relativeTo, c),
                snappedMilliseconds = Math.floor(d / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.MILLI, snappedMilliseconds)
                }
            break;
        case Sch.util.Date.SECOND:
            if (b) {
                var e = Sch.util.Date.getDurationInSeconds(relativeTo, c),
                snappedSeconds = Math.floor(e / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.MILLI, snappedSeconds * 1000)
                } else {
                c.setMilliseconds(0)
                }
            break;
        case Sch.util.Date.MINUTE:
            if (b) {
                var f = Sch.util.Date.getDurationInMinutes(relativeTo, c),
                snappedMinutes = Math.floor(f / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.SECOND, snappedMinutes * 60)
                } else {
                c.setSeconds(0);
                c.setMilliseconds(0)
                }
        ...
        case Sch.util.Date.HOUR:
            if (b) {
                var g = Sch.util.Date.getDurationInHours(this.getStart(), c),
                snappedHours = Math.floor(g / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.MINUTE, snappedHours * 60)
                } else {
                c.setMinutes(0);
                c.setSeconds(0);
                c.setMilliseconds(0)
        ...
        case Sch.util.Date.DAY:
            if (b) {
                var h = Sch.util.Date.getDurationInDays(relativeTo, c),
                snappedDays = Math.floor(h / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.HOUR, snappedDays * 24)
                } else {
                c.clearTime()
                }
            break;
        case Sch.util.Date.WEEK:
            ...
            if (i !== this.weekStartDay) {
                c = Sch.util.Date.add(c, Sch.util.Date.DAY, -(i > this.weekStartDay ? (i - this.weekStartDay) : (7 - i - this.weekStartDay)))
                }
            break;
        case Sch.util.Date.MONTH:
            if (b) {
                var j = Sch.util.Date.getDurationInMonths(relativeTo, c),
                snappedMonths = Math.floor(j / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.MONTH, snappedMonths)
                } else {
                c.clearTime();
                c.setDate(1)
                }
        ...
        case Sch.util.Date.QUARTER:
            c.clearTime();
            c.setDate(1);
            c = Sch.util.Date.add(c, Sch.util.Date.MONTH, -(c.getMonth() % 3));
            break;
        case Sch.util.Date.YEAR:
            if (b) {
                var k = Sch.util.Date.getDurationInYears(relativeTo, c),
                snappedYears = Math.floor(k / increment) * increment;
                c = Sch.util.Date.add(relativeTo, Sch.util.Date.YEAR, snappedYears)
                } else {
    ...
    ceilDate: function(a, b) {
        var c = a.clone();
        b = b !== false;
        var d = b ? this.resolutionIncrement: 1,
        unit = b ? this.resolutionUnit: this.mainUnit,
        doCall = false;
        switch (unit) {
        case Sch.util.Date.DAY:
            if (c.getMinutes() > 0 || c.getSeconds() > 0 || c.getMilliseconds() > 0) {
                doCall = true
            }
            break;
        case Sch.util.Date.WEEK:
            c.clearTime();
            if (c.getDay() !== this.weekStartDay) {
                doCall = true
            }
            break;
        case Sch.util.Date.MONTH:
    ...
    getNext: function(a, b, c) {
        ...
        switch (b) {
        case Sch.util.Date.DAY:
            d = Sch.util.Date.add(d, Sch.util.Date.DAY, c);
            break;
        case Sch.util.Date.WEEK:
            var e = d.getDay();
            d = Sch.util.Date.add(d, Sch.util.Date.DAY, (7 * (c - 1)) + (e < this.weekStartDay ? (this.weekStartDay - e) : (7 - e + this.weekStartDay)));
            break;
        case Sch.util.Date.MONTH:
            d = Sch.util.Date.add(d, Sch.util.Date.MONTH, c);
            d.setDate(1);
            break;
        case Sch.util.Date.QUARTER:
            d = Sch.util.Date.add(d, Sch.util.Date.MONTH, ((c - 1) * 3) + (3 - (d.getMonth() % 3)));
            break;
        case Sch.util.Date.YEAR:
            d = new Date(d.getFullYear() + c, 0, 1);
            break;
        default:
            d = Sch.util.Date.add(a, b, c);
            break
        }
        return d
    },
    ...
    setResolution: function(a, b) {
        this.resolutionUnit = a;
        this.resolutionIncrement = b || 1
    }
});
Ext.ns('Sch');
...
Sch.Tooltip = Ext.extend(Ext.ToolTip, {
    ...
    constructor: function(d) {
        var e = new Sch.ClockTemplate();
        this.renderTo = this.target = document.body;
        if (!this.template) {
            this.template = new Ext.XTemplate('<div class="{[values.valid ? "sch-tip-ok" : "sch-tip-notok"]}">', '{[this.renderClock(values.startDate, values.startText, "sch-tooltip-startdate")]}', '{[this.renderClock(values.endDate, values.endText, "sch-tooltip-enddate")]}', '</div>', {
                compiled: true,
                disableFormats: true,
                renderClock: function(a, b, c) {
                    return e.apply({
                        date: a,
                        text: b,
                        cls: c
        ...
        Sch.Tooltip.superclass.constructor.call(this, d)
        },
    update: function(a, b, c) {
        var d = this.scheduler.getFormattedDate(a),
        endText = this.scheduler.getFormattedEndDate(b, a);
        if (this.mode === 'calendar' && b.getHours() === 0 && b.getMinutes() === 0 && !(b.getYear() === a.getYear() && b.getMonth() === a.getMonth() && b.getDate() === a.getDate())) {
        ...
        if (t) {
            ...
            if (t === this.triggerElement) {
                if (!this.hidden && this.trackMouse) {
                    var a = this.getTargetXY();
                    if (this.constrainToResource) {
                        this.setPagePosition([a[0], this.origXY[1]])
                        } else {
...
Sch.BasicViewPresets = {
    ...
    var a = Sch.PresetManager,
    bvp = Sch.BasicViewPresets;
    for (var p in bvp) {
        if (bvp.hasOwnProperty(p)) {
            a.registerPreset(p, bvp[p])
            }
    }
})();
function schedulerDiagnostics() {
    ...
    if (console && console.log) {
        ...
        b = function(a) {
            schedulerDebugWin.update((schedulerDebugWin.body.dom.innerHTML || '') + a + '<br/>')
            }
    }
    ...
    if (eventStore.getCount() > 0) {
        if (!eventStore.getAt(0).get('StartDate') || !(eventStore.getAt(0).get('StartDate')
            instanceof Date)) {
            b('The eventStore reader is misconfigured - The StartDate field is not setup correctly, please investigate');
            return
        }
        if (!eventStore.getAt(0).get('EndDate') || !(eventStore.getAt(0).get('EndDate')
            instanceof Date)) {
        ...
        eventStore.each(function(r, i) {
            b((i + 1) + '. Start:' + r.get('StartDate') + ', End:' + r.get('EndDate') + ', ResourceId:' + r.get('ResourceId'))
            })
        } else {
...
Sch.util.headerRenderers = function() {
    ...
    return {
        ...
        dayStartEndHours: function(a, b, c) {
            ...
            return String.format('<span class="sch-hdr-start">{0}</span><span class="sch-hdr-end">{1}</span>', a.format('G'), b.format('G'))
            }
    }
} ();
